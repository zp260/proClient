...
-(void)getOLData
{
    ...
    [get GetUrl:url target:self selector:@selector(dataCheck:) parameters:nil];
}
-(void)dataCheck:(NSDictionary *)backData
{
    if (backData.count>0)
    {
        self._programArrays = [[NSArray alloc]initWithArray:[backData objectForKey:@"rows"]];
        NSLog(@"%@",[backData objectForKey:@"rows"]);
    }
    [self._TableView reloadData];
...
{
    return self._programArrays.count;
}
